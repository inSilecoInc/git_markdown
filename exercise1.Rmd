...
The [penguins dataset](https://github.com/allisonhorst/palmerpenguins) contains data on `r nrow(penguins)` individuals, `r length(unique(penguins$species))` species, and from `r length(unique(penguins$island))` islands. Across individuals, the mean flipper length is `r round(mean(penguins$flipper_length_mm, na.rm = TRUE),2)` $mm$.
